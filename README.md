...
- **`ignoreNotFound`** { boolean } (false) - Flag to ignore modules that are not found in storage. When set to true these modules will just be empty entries in the bundle.
...
Object that encapsulates bundle information. All bundles, including shards in the context are instaces of `Bundle`.

- **`name`** { string } - Bundle name.
- **`modules`** { Module[] } - Array of modules in the bundle.
- **`exports`** { string[] } - Array of ids from `modules` that are exported.
- **`content`** { Buffer | string } - Bundle string. This is generally what gets written to disk.
- **`main`** { boolean } - Flag that is true if the bundle is the main bundle, false otherwise.
